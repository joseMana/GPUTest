...
namespace GPUTest
{
    class Program
    {
        ...
        private static void AddUsingGPU()
        {
            ...
            float[] vectorA = Enumerable.Range(1, size).Select(i => (float)i).ToArray();
            float[] vectorB = Enumerable.Range(1, size).Select(i => (float)i * 2).ToArray();
            ...
            ComputeBuffer<float> bufferResult = new ComputeBuffer<float>(context, ComputeMemoryFlags.WriteOnly, size);
            ...
            queue.Read(bufferResult, true, 0, size, handle.AddrOfPinnedObject(), null);
            handle.Free();
